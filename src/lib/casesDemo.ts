...
const STORAGE_KEY = "radonaix_cases_v4";
...
export const SEED: AssuranceCase[] = [
  {
    ...
    description: "412 AIR records show a processed amount below the raw charged amount after mediation. Variance concentrated on NODE-2 between 02:00 and 04:00.",
    ...
    estimatedImpact: 248500, createdAt: iso(0, 2), updatedAt: iso(0, 6),
  ...
  },
  {
    ...
    description: "Three consecutive SDP CDR files absent from the ingest window. Upstream mediation reported successful export for all three.",
    ...
    estimatedImpact: 96200, createdAt: iso(1, 3), updatedAt: iso(0, 11),
  ...
  },
  {
    ...
    title: "AIR vs SDP cross-recon mismatch — 1,204 unmatched",
    description: "Cross correlation between AIR and SDP shows 1,204 transactions present in AIR with no SDP counterpart for the 18 Jun window.",
    ...
    estimatedImpact: 512300, createdAt: iso(2, 4), updatedAt: iso(1, 9),
    evidence: null, trace: trace("AIR", 8, "RAW_ONLY"), comments: [], savedInsights: [],
  },
  {
    id: "c-2028", reference: "CASE-2028",
    title: "Record sequence gap — AIR NODE-4 (18,442 records)",
    description: "Record sequence check found a contiguous gap on NODE-4. Suspected collector restart during the 17 Jun maintenance window.",
    ...
    estimatedImpact: 31800, createdAt: iso(3, 8), updatedAt: iso(1, 14),
  ...
  },
  {
    ...
    description: "Decoder rejected the file on a malformed header record. File quarantined; 0 records ingested.",
    ...
    estimatedImpact: 74400, createdAt: iso(3, 5), updatedAt: iso(0, 8),
...
export const fmtMoney = (n: number) => `₹${new Intl.NumberFormat("en-IN").format(Math.round(n))}`;
